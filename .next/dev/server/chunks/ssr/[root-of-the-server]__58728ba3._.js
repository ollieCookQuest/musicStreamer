...
function FullScreenPlayer({ onClose }) {
    const { currentGroup, playbackState, volume, setVolume, refreshState, setIsExpanded } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$components$2f$sonos$2d$provider$2e$tsx__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useSonos"])();
    ...
    const togglePlayback = async ()=>{
        ...
        try {
            ...
            if (res.ok) {
                ...
                console.error("Control failed:", errorData);
            }
        } catch (error) {
    ...
    const skip = async (direction)=>{
        if (!currentGroup) return;
        try {
            const res = await fetch(`/api/sonos/control?groupId=${currentGroup.id}&command=${direction}`, {
                method: "POST"
            });
            if (res.ok) {
                // Immediately refresh, then refresh again after a delay
                ...
                const errorData = await res.json().catch(()=>({}));
                console.error("Skip failed:", errorData);
            }
        } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__58728ba3._.js.map