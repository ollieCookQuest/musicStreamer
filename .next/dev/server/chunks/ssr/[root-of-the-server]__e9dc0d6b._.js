...
function FullScreenPlayer({ onClose }) {
    ...
    const handleVolumeChange = async (value)=>{
        ...
        // If zones are grouped (single selected zone), control the group volume
        // This ensures grouped zones have synchronized volume
        if (selectedZoneIds.length === 1) {
            const groupId = selectedZoneIds[0];
            try {
                ...
                console.error("Failed to set volume for grouped zone:", error);
            }
        } else if (selectedZoneIds.length === 0) {
            // Fallback to current group
            setVolume(newVolume);
        } else {
            // If multiple zones selected but not grouped, group them first
            await handleGroupZones();
            // Then set volume for the grouped zone
            const groupId = selectedZoneIds[0];
            if (groupId) {
                try {
                    const res = await fetch(`/api/sonos/control?groupId=${groupId}&volume=${newVolume}`, {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e9dc0d6b._.js.map